...
import { fullFavoriteTokensAtom } from "@/lib/store/atoms/cookieStorage";
...
export default function FavoriteTokenListClient({ items }: Props) {
  const [fullFavorites, setFullFavorites] = useAtom(fullFavoriteTokensAtom);

  useEffect(() => {
    setFullFavorites(items);
  }, [items]);

  return (
    <>
      <ul className="flex flex-wrap gap-4 justify-center  w-full">
        {fullFavorites.map((token) => (
          <li key={`${token.chainId}-${token.address}`}>
            ...
            >
              ...
              <FavoriteIcon token={token} isFavorite={true} />
            </Link>
          </li>
        ))}
      </ul>
